    ...
    return BlocBuilder<LoginCubit, LoginStates>(
      builder: (context, state) {
        if (state is LoginInitStates) {
                    ...
                    Custom_Bottom(
                      function: () async {
                        if (key.currentState!.validate()) {
                         ...
                         cubit.checkuser(emailcontrolar.text, passwordcontrolar.text);
                        }
                      },
                      widget: Text(
                        'Login',
                        style: TextStyle(
                            fontSize:
                                ResponsiveSize(context: context, size: 20).size,
                            fontWeight: FontWeight.bold,
                            color: Colors.white),
                      ),
                    ),
          ...
          return HomeViews(phone: cubit.phonee,);
            ...
            ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    backgroundColor: Colors.red,
                                      content:
                                          const Text("You login in another device")));
          },);
          return LoginView();
        }else{
          ...
          Future.delayed(Duration(seconds: 2)).then((value) {
            ScaffoldMessenger.of(context).showSnackBar(
                                  SnackBar(
                                    backgroundColor: Colors.red,
                                      content:
                                          const Text("phone or password rong")));
          },);
          return LoginView();
